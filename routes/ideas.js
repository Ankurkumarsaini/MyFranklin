		...
		responseFromAPI => {
			...
			responseFromAPI.on('end', () => {
				...
				*/
				...
				return res.json({"displayText":"ANkur"});
				//return res.send({"fulfillmentMessages": [{"text": {"text": ["Text response from webhook"]}}]});
    ...
    return rp(options)
	 ...
	 text +='\n\n';	
		...
		try 
			...
			token: process.env.TOKEN,
			channel: 'D01F46BL5QE',
			text:"*Below is the details of Issue*",
...
function backlogIssueCreateHandler(req,res,next){	
    ...
    return rp(options)
        .then(response => {
			console.log(response);
	               var TicketNo=response.issueKey;
			 try 
			    {			        
				const result = app.client.chat.postMessage({
				token: process.env.TOKEN,
				...
				attachments:'[{"color": "#3AA3E3","text":"Your backlog Ticket No is :-'+ TicketNo +'"}]',					
				  });
			}catch (error) {
...
function backlogCurrentUserHandler(req, res, next){
	...
	console.log(req.body.originalDetectIntentRequest.payload.data.event.user);
